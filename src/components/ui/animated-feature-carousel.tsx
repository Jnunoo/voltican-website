...
const placeholderImage = (text = "Image") =>
  `https://placehold.co/600x400/003e6a/ffffff?text=${text}`;
...
function FeatureCard({
  ...
  return (
    ...
    >
      <div className="relative w-full overflow-hidden rounded-3xl border border-brand-ice/20 bg-white shadow-lg">
        <div className="m-10 min-h-[450px] w-full">
          <AnimatePresence mode="wait">
            ...
            >
              <motion.div
                className="text-sm font-semibold uppercase tracking-wider text-brand-blue"
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{
                  delay: 0.05,
              ...
              <motion.h2
                className="text-2xl font-heading font-bold tracking-tight text-brand-navy md:text-3xl"
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{
                  delay: 0.1,
          ...
          return (
            ...
            >
              <button
                type="button"
                className={cn(
                  "group flex items-center gap-2.5 rounded-full px-3.5 py-1.5 text-sm font-medium transition-colors duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-brand-blue",
                  isCurrent
                    ? "bg-brand-blue text-white"
                    : "bg-surface-light text-brand-navy hover:bg-brand-ice/50"
                )}
                ...
                <span
                  className={cn(
                    ...
                    isCompleted
                      ...
                      : "bg-surface-muted text-brand-navy group-hover:bg-brand-ice"
                  )}
                >
                  {isCompleted ? (
...
const defaultClasses = {
  img: "rounded-xl border border-brand-ice/30 shadow-2xl shadow-brand-navy/10",
...
export function FeatureCarousel({
  ...
  const stepItems = customSteps ?? defaultSteps;
  const { currentNumber: step, setStep } = useNumberCycler(stepItems.length);
  ...
  return (
    <div className="flex flex-col gap-12 w-full max-w-4xl mx-auto p-4">
      <FeatureCard {...props} step={step} steps={stepItems}>
        <AnimatePresence mode="wait">
          <motion.div
            key={step}
      ...
      >
        <StepsNav current={step} onChange={setStep} steps={stepItems} />
      </motion.div>
    </div>
  );
}
